...
bool Coin::getMintedAmount(plc::MintingLimits & limits) const
{
    if (m_storage.empty())
    {
        // empty storage? this is the norm (malysheva style)
        return false;
    }

    CDataStream existing(m_storage, SER_DISK, 0);
    uint32_t version = 0;
    existing >> version;
    if (version & COINS_STORAGE_MINTING_LIMITS)
    {
        existing >> limits.mintingTimestamp >> limits.mintingCurrent;
    }
...
bool Coin::updateMintedAmount(const plc::MintingLimits & limits)
{
    CDataStream stream(SER_DISK, 0);
    uint32_t version = COINS_STORAGE_MINTING_LIMITS | COINS_STORAGE_DAILY_LIMITS;
    stream << version
           << limits.mintingTimestamp << limits.mintingCurrent
           << limits.dailyTimestamp << limits.maxLoadCurrent;
