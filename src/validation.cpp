...
#if defined(NDEBUG)
# error "PLC Ultima cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "PLC Ultima Signed Message:\n";
...
//******************************************************************************
// returns pair, transfer amount - burned amount
//******************************************************************************
// static
                              ...
                              bool dryrun)
{
    ...
    if (!ContextualCheckTransaction(tx, state, chainActive.Height()))
        return false; // state filled in by ContextualCheckTransaction
...

    {
        ...
        CAmount nRefill = view.GetUsedMoneyBoxAmount(tx);
        ...
        CTxMemPoolEntry entry(ptx, nFees, nRefill,
                              nAcceptTime, chainActive.Height(),
    ...
    GetMainSignals().NotifyTransaction(ptx, NULL, CMainSignals::SYNC_TRANSACTION_NOT_IN_BLOCK,
                                       CValidationInterface::txAcceptToMempool);
                        ...
                        bool dryrun)
{
    ...
    bool res = AcceptToMemoryPoolWorker(chainparams, pool, state, tx, fLimitFree, pfMissingInputs, nAcceptTime, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, coins_to_uncache, dryrun);
    if (!res) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx, bool fLimitFree,
                        ...
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee, bool dryrun)
{
    const CChainParams& chainparams = Params();
    return AcceptToMemoryPoolWithTime(chainparams, pool, state, tx, fLimitFree, pfMissingInputs, GetTime(), plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, dryrun);
}
...
void UpdateCoins(const CTransaction & tx,
                 CCoinsViewCache & inputs, CTxUndo & txundo, int nHeight, unsigned int blockTime)
{
    // mark inputs spent
    if (!tx.IsCoinBase())
    {
        CAmount inAmount     = 0;
        CAmount outAmount    = 0;
        CAmount boxInAmount  = 0;
        CAmount boxOutAmount = 0;

    //     txundo.vprevout.reserve(tx.vin.size());
        for (const CTxIn & txin : tx.vin)
        {
            std::vector<plc::Certificate> certs;
            if (txundo.m_certs.size() == 0 && isMintingScript(txin.scriptSig, certs))
            {
                txundo.m_certs     = certs;
                txundo.m_isMinting = true;
            }

            const Coin & coin = inputs.AccessCoin(txin.prevout);

            if (coin.out.scriptPubKey == Params().moneyBoxAddress())
            {
                boxInAmount += coin.out.nValue;
            }
            else
            {
                inAmount += coin.out.nValue;
            }
        ...
        {
            if (txout.scriptPubKey == Params().moneyBoxAddress())
            {
                boxOutAmount += txout.nValue;
            }
            else
            {
                outAmount += txout.nValue;
            }

        } // for (const CTxOut & txout : tx.vout)

        if (txundo.m_isMinting)
        {
            // uint32_t now = static_cast<uint32_t>(time(nullptr));
            // uint32_t now = chainActive[nHeight]->nTime;
            uint32_t now = blockTime;

            plc::Certificate & cert = txundo.m_certs.back();
            plc::Validator validator;
            plc::CertParameters params;
            if (!validator.loadCert(cert, params))
            {
                LogPrintf("%s: ERROR cert not found <%s:%d>\n", __func__,
                       cert.txid.ToString(), cert.vout);
            }
            else if (params.flags & plc::hasMintingLimit || params.flags & plc::hasMaxload)
            {
                // calc minting amount for this tx
                if (outAmount > inAmount)
                {
                    txundo.m_mintingAmount = outAmount - inAmount;

                    COutPoint op(txundo.m_certs.back().txid, txundo.m_certs.back().vout);

                    Coin coin;
                    assert(inputs.GetCoin(op, coin));
                    if (!coin.getMintedAmount(params.limits))
                    {
                        LogPrintf("%s: WARN cert with empty storage <%s:%d>\n", __func__,
                               cert.txid.ToString(), cert.vout);
                    }

                    bool isDataUpdated = false;
                    if (params.flags & plc::hasMintingLimit)
                    {
                        params.limits.mintingTimestamp = now;
                        params.limits.mintingCurrent  += txundo.m_mintingAmount;
                        isDataUpdated = true;
                    }
                    if (params.flags & plc::hasMaxload)
                    {
                        if (now <= params.limits.dailyTimestamp + oneDay)
                        {
                            params.limits.maxLoadCurrent += inAmount;
                        }
                        else
                        {
                            params.limits.dailyTimestamp = now;
                            params.limits.maxLoadCurrent = inAmount;
                        }
                        isDataUpdated = true;
                    }
                    ...
                    if (isDataUpdated)
                    {
                        coin.updateMintedAmount(params.limits);
                        inputs.AddCoin(op, std::move(coin), true);
                    }
...
void UpdateCoins(const CTransaction & tx,
                 CCoinsViewCache & inputs, int nHeight, unsigned int blockTime)
{
    CTxUndo txundo;
    UpdateCoins(tx, inputs, txundo, nHeight, blockTime);
}

bool CScriptCheck::operator()(const CCoinsViewCache * view) {
    const CScript &scriptSig = ptxTo->vin[nIn].scriptSig;
    const CScriptWitness *witness = &ptxTo->vin[nIn].scriptWitness;
    return VerifyScript(scriptSig, scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, amount, view, cacheStore, *txdata), &error);
}
...
static DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    {
        const CTransaction &tx = *(block.vtx[static_cast<size_t>(i)]);
        ...
        if (i > 0)
        {
            ...
            if (txundo.m_isMinting)
            {
                const plc::Certificate & cert = txundo.m_certs.back();

                COutPoint out(cert.txid, cert.vout);
                Coin coin;
                if (!view.GetCoin(out, coin))
                {
                    // unrealistic situation, but everyone should be happy
                    error("DisconnectBlock(): no certificate for minting tx");
                    return DISCONNECT_FAILED;
                }
                ...
                for (const Coin & coin : txundo.vprevout)
                {
                    if (coin.out.scriptPubKey != Params().moneyBoxAddress())
                    {
                        userIn += coin.out.nValue;
                    }
                }

                CAmount mintedCoins = txundo.m_mintingAmount;

                plc::MintingLimits limits;
                if (coin.getMintedAmount(limits))
                {
                    limits.mintingCurrent = ((mintedCoins >= limits.mintingCurrent) ?
                                              0 : limits.mintingCurrent - mintedCoins);

                    limits.maxLoadCurrent = ((userIn >= limits.maxLoadCurrent) ?
                                              0 : limits.maxLoadCurrent - userIn);
                }

                coin.updateMintedAmount(limits);
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    {
        if (out.nValue == 0 && out.scriptPubKey[0] == OP_RETURN)
        {
            // allow OP_RETURN only where amount is == 0
            continue;
        }
        ...
        if (out.nValue == 0)
        {
            return state.DoS(100,
                             error("ConnectBlock(): zero amount in coinbase transaction"),
                                   REJECT_INVALID, "bad-cb-amount");
        }

        if (out.scriptPubKey == chainparams.moneyBoxAddress())
        {
            if (out.nValue > Params().awardGranularity(pindex->nHeight))
            {
                return state.DoS(100,
                                 error("ConnectBlock(): invalid PLCU granularity in coinbase"),
                                       REJECT_INVALID, "bad-cb-amount");
            }
    ...
    if (pindex->nHeight <= chainparams.GetConsensus().countOfInitialAwardBlocks)
    {
        if (nRefill != 0)
        {
            return state.DoS(100,
    ...
    if (countOfAward > 1 + nRefill / chainparams.awardGranularity(pindex->nHeight))
    {
        return state.DoS(100,
                         error("ConnectBlock(): too many refill outputs in coinbase (%d)",
                               countOfAward),
                               REJECT_INVALID, "bad-box-count");
    }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (fCheckpointsEnabled) {
        ...
        // MapBlockIndex.
        CBlockIndex* pcheckpoint = Checkpoints::GetLastCheckpoint(params.Checkpoints().mapCheckpoints);
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    for (const auto& tx : block.vtx)
    {
        ...
        if (!ContextualCheckTransaction(*tx, state, nHeight))
        {
            return state.Invalid(false, state.GetRejectCode(), state.GetRejectReason(),
                                 strprintf("Contextual transaction check failed (tx hash %s) %s", tx->GetHash().ToString(), state.GetDebugMessage()));
        }
    }
