...
void TxToJSON(const CTransaction& tx, const uint256 & hashBlock, UniValue& entry, const bool includeHex = false)
{
    ...
    TxToUniv(tx, uint256(), entry, includeHex, RPCSerializationFlags());
...
UniValue getrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (hash == Params().GenesisBlock().vtx[0]->GetHash())
    {
...
//******************************************************************************
//******************************************************************************
void parseJsonInputObject(const UniValue & param, CTxIn & in,
                          const bool txIsLocked = false,
                          const bool rbfOptIn = false)
{
    ...
    uint32_t nSequence = rbfOptIn   ? MAX_BIP125_RBF_SEQUENCE :
                         txIsLocked ? std::numeric_limits<uint32_t>::max() - 1 :
    ...
    in = CTxIn(COutPoint(txid, nOutput), CScript(), nSequence);
}

//******************************************************************************
//******************************************************************************
void parseJsonInputsArray(const UniValue & param, CMutableTransaction & rawTx, const bool rbfOptIn)
{
    UniValue inputs = param.get_array();
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++)
    {
        ...
        CTxIn in;
        parseJsonInputObject(input, in, rawTx.nLockTime, rbfOptIn);
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the plcultima address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
    ...
    parseJsonInputsArray(request.params[0], rawTx, rbfOptIn);
    ...
    for (const std::string& name_ : addrList)
    {
        ...
        else
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid plcultima address: ")+name_);
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) plcultima address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
    ...
    UniValue result(UniValue::VOBJ);
    TxToUniv(CTransaction(std::move(mtx)), uint256(), result, true);
...
UniValue signrawtransaction(const JSONRPCRequest& request)
{
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); i++)
    {
        ...
        if (!VerifyScript(txin.scriptSig, prevPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&txConst, i, amount, &view), &serror)) {
...
UniValue sendrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "sendrawtransaction \"hexstring\" ( allowhighfees dryrun )\n"
            "\nSubmits raw transaction (serialized, hex-encoded) to local node and network.\n"
            "\nAlso see createrawtransaction and signrawtransaction calls.\n"
            "\nArguments:\n"
            "1. \"hexstring\"    (string, required) The hex string of the raw transaction)\n"
            "2. allowhighfees    (boolean, optional, default=false) Allow high fees\n"
            "3. dryrun           (boolean, optional, default=false) Dry run (only check the transaction, without sending)\n"
            "\nResult:\n"
    ...
    uint256 hashTx = sendTransaction(request.params[0].get_str(), nMaxRawTxFee, isDryrun);
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "rawtransactions",    "sendrawtransaction",     &sendrawtransaction,     false, {"hexstring","allowhighfees","dryrun"} },
