...
UniValue generateBlocks(const CKeyStore & keystore,
                        ...
                        int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    {
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(keystore, Params()).CreateNewBlock(coinbaseScript->subsidyDestination));
...
UniValue generate(const JSONRPCRequest& request)
{
    ...
    throw std::runtime_error("Wallet needed");
#else
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated plcultima to.\n"
...
// NOTE: Unlike wallet RPC (which use PLCU values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "PLC Ultima is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "PLC Ultima is downloading blocks...");
    ...
    if (pblock->vtx[0]->vout.size() >= 2)
    {
        ...
        {
            values.push_back((int64_t)pblock->vtx[0]->vout[i].nValue);
        }
        result.push_back(Pair("coinbasevalue", values));
    }
    else if (pblock->vtx[0]->vout.size() >= 1)
    {
...
UniValue estimaterawfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "      \"feerate\" : x.x,        (numeric, optional) estimate fee-per-kilobyte (in PLCU)\n"
