...
UniValue stop(const JSONRPCRequest& jsonRequest)
{
    ...
    if (jsonRequest.fHelp || jsonRequest.params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "\nStop PLC Ultima server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "PLC Ultima server stopping";
}
...
void JSONRPCRequest::parse(const UniValue& valRequest)
{
    ...
    {
        std::string sparams = valParams.write();
        LogPrint(BCLog::RPC, "ThreadRPCServer method=%s, params=%s\n", SanitizeString(strMethod), SanitizeString(sparams));
    }
...
{
    return "> plcultima-cli " + methodname + " " + args + "\n";
}
