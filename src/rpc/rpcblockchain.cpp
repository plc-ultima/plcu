...
extern void TxToJSON(const CTransaction & tx, const uint256 & hashBlock, UniValue & entry, const bool includeHex = false);
...
UniValue getblock(const JSONRPCRequest& request)
{
    ...
    if (!request.params[1].isNull())
    {
        ...
        {
            verbosity = std::atoi(request.params[1].get_str().c_str());
        }
        else
        {
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of plcultima addresses\n"
    ...
    if (!coin.m_storage.empty())
    {
        ret.push_back(Pair("storage", HexStr(coin.m_storage)));
    }
