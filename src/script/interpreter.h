...
public:
    ...
    virtual bool CheckReward(const std::vector<plc::Certificate> & /*certs*/,
...
private:
    ...
    mutable bool m_rewardAlreadyChecked;
    mutable bool m_rewardCheckResult;

    mutable std::map<uint32_t, uint32_t> m_inOutRequired;
...
public:
    TransactionSignatureChecker(const CTransaction* txToIn,
                                ...
                                const CCoinsViewCache * viewIn = nullptr)
        : txTo(txToIn)
        , nIn(nInIn)
        , amount(amountIn)
        , txdata(NULL)
        , view(viewIn)
        , m_rewardAlreadyChecked(false)
        , m_rewardCheckResult(false)
    {}

    TransactionSignatureChecker(const CTransaction* txToIn,
                                unsigned int nInIn,
                                const CAmount& amountIn,
                                const CCoinsViewCache * viewIn,
    ...
    bool CheckReward(const std::vector<plc::Certificate> & certs,
                     const plc::CertParameters & params,
                     ScriptError * serror) const;
...
public:
    MutableTransactionSignatureChecker(const CMutableTransaction* txToIn,
                                       ...
                                       const CAmount& amount,
                                       const CCoinsViewCache * viewIn = nullptr)
        : TransactionSignatureChecker(&txTo, nInIn, amount, viewIn), txTo(*txToIn)
    {}
};
