...
void CWallet::NotifyTransaction(const CTransactionRef & ptx, const CBlockIndex * /*pindex*/,
                                int /*posInBlock*/, const SyncTransactionReason & /*reason*/)
{
    LOCK2(cs_main, cs_wallet);
    SyncTransaction(ptx);
}
...
uint32_t getLockTime()
{
    ...
    uint32_t nLockTime = chainActive.Height();
    ...
    assert(nLockTime <= (unsigned int)chainActive.Height());
...
//******************************************************************************
//******************************************************************************
bool getScriptChange(CReserveKey & reservekey, std::string & strFailReason,
                     const CCoinControl & coinControl, CScript & scriptChange)
{
    ...
    else if (!coinControl.fReturnChangeToAddressFromInputSet)
    {
...
//******************************************************************************
//******************************************************************************
CAmount calcBurnAmount(const CAmount & transferAmount,
                       std::map<CScript, CAmount> & graveAmounts)
{
                                ...
                                const CCoinControl & coin_control, bool sign)
{
    ...
    if (!getScriptChange(reservekey, strFailReason, coin_control, scriptChange))
    {
        return false;
    }
    ...
    while (true)
    {
        ...
        if (needSelectAgain)
        {
            if (!SelectCoins(vAvailableCoins, neededAmount,
                             setCoins, selectedAmount, &coin_control))
            {
        ...
        if (!coin_control.fReturnChangeToAddressFromInputSet || nSubtractFeeFromAmount > 0)
        {
            ...
            burnedAmount = (is_taxFree || is_sendToSelf) ?
                        0 :
                        calcBurnAmount(static_cast<CAmount>((newTransferAmount - feeAmount) /
                                                            (1 + Params().gravePercent())),
                                                                            graveAmounts);
            transferAmount = std::max(CAmount(0), newTransferAmount - burnedAmount - feeAmount);
        }
        ...
        // change
        CAmount changeAmount = selectedAmount - neededAmount;
...

        {
            ...
            for (const auto & recipient : vecSend)
            {
                ...
                CTxOut txout(recipient.nAmount - subAmount, recipient.scriptPubKey);
            ...
            {
                for (const std::pair<const CScript, CAmount> & g : graveAmounts)
                {
                    if (g.second > 0)
                    {
                        txNew.vout.emplace_back(g.second, g.first);
                    }
                }
            }
        ...
        // change
        {
            ...
            else
            {
                ...
                if (IsDust(newTxOut, discardRate))
                {
                    changePosInOut = -1;
                }
                else
                {
                    if (changePosInOut == -1)
                    {
                        ...
                        changePosInOut = GetRandInt(txNew.vout.size()+1);
                    }
                    else if ((unsigned int)changePosInOut > txNew.vout.size())
                    {
                        strFailReason = _("Change index out of range");
                        return false;
                    }

                    std::vector<CTxOut>::iterator position = txNew.vout.begin() + changePosInOut;
        ...
        // fee
        {
            ...
            nBytes = GetVirtualTransactionSize(txNew) + (is_taxFree ? 206 : 0);
            ...
            CAmount feeNeeded = GetMinimumFee(nBytes, coin_control, ::mempool, ::feeEstimator, &feeCalc);

            // If we made it here and we aren't even able to meet the relay fee on the next pass, give up
            // because we must be at the maximum allowed fee.
            if (feeNeeded < ::minRelayTxFee.GetFee(nBytes))
            {
        ...
        nChangePosInOut = changePosInOut;
        nFeeRet         = feeAmount;
        break;
    }
    ...
    LogPrintf("Fee Calculation: Fee:%d Burned:%d Bytes:%u Needed:%d Tgt:%d (requested %d) Reason:\"%s\" Decay %.5f: Estimation: (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out) Fail: (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out)\n",
              nFeeRet, burnedAmount, nBytes, nFeeRet,
              feeCalc.returnedTarget, feeCalc.desiredTarget,
...
void CWallet::GetScriptForMining(std::shared_ptr<CReserveScript> &script)
{
    ...
    script = rKey;
    script->subsidyDestination = CScript() << ToByteVector(pubkey) << OP_CHECKSIG;
}
...
bool CMerkleTx::AcceptToMemoryPool(const CAmount& nAbsurdFee, CValidationState& state)
{
    ...
    return ::AcceptToMemoryPool(mempool, state, tx, is_limitFree, nullptr, nullptr, false, nAbsurdFee);
}
