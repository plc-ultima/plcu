...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PLC Ultima address or script");
    }
...
UniValue getpubkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            "getpubkey \"address\" ( compressed )\n"
            "\nReveals the public key corresponding to 'address'.\n"
            "\nArguments:\n"
            "1. \"address\"   (string, required) The plcultima address for the public key\n"
            "2. compressed    (bool, optional, default=true) If true, return compressed key, otherwise return uncompressed\n"
            "\nResult:\n"
    ...
    if (request.params.size() > 1)
    {
        ...
        {
            isCompressed = (request.params[1].get_int() > 0);
        }
        else if(request.params[1].isBool())
        {
            isCompressed = request.params[1].isTrue();
        }
        else
        {
            throw JSONRPCError(RPC_TYPE_ERROR, "Invalid type provided. Compressed parameter must be a boolean.");
        }
    }
    ...
    CPubKey pub = vchSecret.GetPubKey();
    if (!isCompressed && pub.IsCompressed())
    {
        if (!pub.Decompress())
        {
            throw JSONRPCError(RPC_WALLET_ERROR, "Invalid pubkey (decompress failed)");
        }
    }
    else if (isCompressed && !pub.IsCompressed())
    {
        throw JSONRPCError(RPC_WALLET_ERROR, "Invalid pubkey (compress opration not implemented)");
    }
    return HexStr(pub.begin(), pub.end());
}
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The plcultima address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to plcultimad)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    // produce output
    file << strprintf("# Wallet dump created by PLC Ultima %s\n", CLIENT_BUILD);
