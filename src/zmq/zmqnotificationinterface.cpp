...
CZMQNotificationInterface* CZMQNotificationInterface::Create()
{
    ...
    factories["pubjsonfullblock"] = CZMQAbstractNotifier::Create<CZMQPublishFullBlockNotifier>;
...
void CZMQNotificationInterface::NotifyTransaction(const CTransactionRef & ptx, const CBlockIndex * pindex,
                                                  int posInBlock, const SyncTransactionReason & reason)
{
    ...
    {
        CZMQAbstractNotifier *notifier = *i;
        if (notifier->NotifyTransaction(ptx, pindex, posInBlock, reason))
        {
            i++;
        }
        else
...
void CZMQNotificationInterface::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindexConnected, const std::vector<CTransactionRef>& vtxConflicted)
{
    ...
    for (uint32_t i = 0; i < pblock->vtx.size(); ++i)
    {
        // Do a normal notify for each transaction added in the block
        const CTransactionRef & ptx = pblock->vtx[i];
        NotifyTransaction(ptx, pindexConnected, i, txAcceptToMempool);
    }
}
...
void CZMQNotificationInterface::BlockDisconnected(const std::shared_ptr<const CBlock>& pblock)
{
    for (const CTransactionRef& ptx : pblock->vtx)
    {
        ...
        NotifyTransaction(ptx, nullptr, -1, txAcceptToMempool);
    }
}
