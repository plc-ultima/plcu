...
BOOST_AUTO_TEST_CASE(rpc_rawsign)
{
    ...
    r = CallRPC(std::string("createrawtransaction ")+prevout+" "+
      "{\"U1xPcY2EAxn2UEqTQi7ZyWg9e4h7kxKgT3M1P\":11}");
    std::string notsigned = r.get_str();
    std::string privkey1 = "\"Aw4UuZNujsF1uRedHu2sMLqcwJYDe1UYMc5pzCCQSqiyjT7iNXJZ7WW\"";
    std::string privkey2 = "\"Aw4UrggHzvqJiawvmpicT4LgtAYCx9TPep2r7nNhUB4y1T9Jy3T4erE\"";
...
BOOST_AUTO_TEST_CASE(json_parse_errors)
{
    ...
    BOOST_CHECK_THROW(ParseNonRFCJSONValue("1.0]"), std::runtime_error);
    // PLCU addresses should fail parsing
...
BOOST_AUTO_TEST_CASE(rpc_ban)
{
    ...
    BOOST_CHECK_NO_THROW(r = CallRPC(std::string("setban 127.0.0.0/24 add 1923264000 true")));
    BOOST_CHECK_NO_THROW(r = CallRPC(std::string("listbanned")));
    ar = r.get_array();
    o1 = ar[0].get_obj();
    ...
    BOOST_CHECK_EQUAL(banned_until.get_int64(), 1923264000); // absolute time check
...
BOOST_AUTO_TEST_CASE(rpc_convert_values_generatetoaddress)
{
    ...
    BOOST_CHECK_NO_THROW(result = RPCConvertValues("generatetoaddress", {"101", "P4tUNeYphUn8YW4dTQc2RtsbJPhywFJZvP8S"}));
    BOOST_CHECK_EQUAL(result[0].get_int(), 101);
    BOOST_CHECK_EQUAL(result[1].get_str(), "P4tUNeYphUn8YW4dTQc2RtsbJPhywFJZvP8S");

    BOOST_CHECK_NO_THROW(result = RPCConvertValues("generatetoaddress", {"101", "P4tLKrEMxWZWbcXmrkJ8oAkKhGCTwzcVBuVs"}));
    BOOST_CHECK_EQUAL(result[0].get_int(), 101);
    BOOST_CHECK_EQUAL(result[1].get_str(), "P4tLKrEMxWZWbcXmrkJ8oAkKhGCTwzcVBuVs");

    BOOST_CHECK_NO_THROW(result = RPCConvertValues("generatetoaddress", {"1", "P4taAaZbN8Sk1FKjedBbYuFSrwXV5KxuaWwV", "9"}));
    BOOST_CHECK_EQUAL(result[0].get_int(), 1);
    BOOST_CHECK_EQUAL(result[1].get_str(), "P4taAaZbN8Sk1FKjedBbYuFSrwXV5KxuaWwV");
    BOOST_CHECK_EQUAL(result[2].get_int(), 9);

    BOOST_CHECK_NO_THROW(result = RPCConvertValues("generatetoaddress", {"1", "P4tVuwQDqCo5M7wy1gVyYBcUybMdyUxgRNhB", "9"}));
    BOOST_CHECK_EQUAL(result[0].get_int(), 1);
    BOOST_CHECK_EQUAL(result[1].get_str(), "P4tVuwQDqCo5M7wy1gVyYBcUybMdyUxgRNhB");
    BOOST_CHECK_EQUAL(result[2].get_int(), 9);
}
