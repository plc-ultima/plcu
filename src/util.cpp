...
const char * const BITCOIN_CONF_FILENAME = "plcultima.conf";
const char * const BITCOIN_PID_FILENAME = "plcultima.pid";
...
#else
    const char* pszModule = "plcultima";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PLC-Ultima";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PLC-Ultima";
#else
    // Unix
    return pathRet / ".plcultima";
#endif
#endif
}
