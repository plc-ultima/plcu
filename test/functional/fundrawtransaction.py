...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        dec_tx  = self.nodes[2].decoderawtransaction(rawtx)
        assert_equal(utx['txid'], dec_tx['vin'][0]['txid'])
        assert_raises_rpc_error(None, "changeAddress must be a valid plcultima address", self.nodes[2].fundrawtransaction, rawtx, {'changeAddress':'foobar'})
        ...
        # PLCU fundrawtransaction returns change to input address, not to new one - no keepool needed
        ...
        self.nodes[1].sendtoaddress(self.nodes[0].getnewaddress(), rest, "", "", True)
        self.sync_all()
        self.nodes[0].generate(1)
        self.sync_all()
