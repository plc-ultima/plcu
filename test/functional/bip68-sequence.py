...
class BIP68Test(BitcoinTestFramework):
    ...
    def test_sequence_lock_unconfirmed_inputs(self):
        ...
        assert_in(tx2.hash, self.nodes[0].getrawmempool())
        ...
        self.nodes[0].setmocktime(cur_time+600)
        self.nodes[0].generate(1)
        assert_not_in(tx2.hash, self.nodes[0].getrawmempool())

        # Now that tx2 is not in the mempool, a sequence locked spend should
        # succeed
        tx3 = test_nonzero_locks(tx2, self.nodes[0], self.relayfee, use_height_lock=False)
        assert_in(tx3.hash, self.nodes[0].getrawmempool())

        self.nodes[0].generate(1)
        assert_not_in(tx3.hash, self.nodes[0].getrawmempool())

        # One more test, this time using height locks
        tx4 = test_nonzero_locks(tx3, self.nodes[0], self.relayfee, use_height_lock=True)
        assert_in(tx4.hash, self.nodes[0].getrawmempool())

        # Now try combining confirmed and unconfirmed inputs
        tx5 = test_nonzero_locks(tx4, self.nodes[0], self.relayfee, use_height_lock=True)
        assert_not_in(tx5.hash, self.nodes[0].getrawmempool())
        ...
        assert_not_in(tx4.hash, self.nodes[0].getrawmempool())
        assert_in(tx3.hash, self.nodes[0].getrawmempool())
        ...
        assert_not_in(tx3.hash, mempool)
