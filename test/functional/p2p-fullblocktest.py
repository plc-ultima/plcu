...
class FullBlockTest(ComparisonTestFramework):
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), solve=True):
        ...
        spendamount = spend.tx.vout[spend.n].nValue - 1 if spend != None else 0  # all but one satoshi to fees
        coinbase = create_coinbase(height, self.coinbase_pubkey, spendamount)
    ...
    def get_tests(self):
        ...
        # Now try a too-large-coinbase script (in plcultima max length is 256 instead of 100 in bitcoin)
        tip(15)
        b28 = block(28, spend=out[6])
        b28.vtx[0].vin[0].scriptSig = b'\x00' * 257
        b28.vtx[0].rehash()
        b28 = update_block(28, [])
        yield rejected(RejectResult(16, b'bad-cb-length'))
        ...
        tip(23)
        b30 = block(30)
        b30.vtx[0].vin[0].scriptSig = b'\x00' * 256
        b30.vtx[0].rehash()
        b30 = update_block(30, [])
        yield accepted()
        save_spendable_output()
        ...
        # NOTE: in regtest we don't require block solve - it will be accepted
        # tip(44)
        ...
        tip(65)
        chunk1 = 600000 # (more than limit 500000)
        b69 = block(69, additional_coinbase_value = max(0, chunk1 - 500000))
        tx = create_and_sign_tx(out[20].tx, out[20].n, out[20].tx.vout[0].nValue - chunk1*2)
        update_block(69, [tx])
        yield accepted()
        save_spendable_output()
