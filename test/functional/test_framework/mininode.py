...
class CTransaction(object):
    def __init__(self, tx=None):
        if tx is None:
            ...
            self.nActiveTime = tx.nActiveTime
    ...
    def deserialize(self, f):
        ...
        if self.nVersion >= 3:
            self.nActiveTime = struct.unpack("<I", f.read(4))[0]
        self.sha256 = None
        self.hash = None
    ...
    def serialize_without_witness(self):
        ...
        if self.nVersion >= 3:
            r += struct.pack("<I", self.nActiveTime)
        return r
    ...
    def __repr__(self):
        return "CTransaction(nVersion=%i vin=%s vout=%s wit=%s nLockTime=%i nActiveTime=%i sha256=%064x)" \
            % (self.nVersion, repr(self.vin), repr(self.vout), repr(self.wit), self.nLockTime,
               self.nActiveTime if self.nActiveTime is not None else 0, self.sha256 if self.sha256 is not None else 0)
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            self.scrypt256 = uint256_from_str(plc_cryptonight.getPoWHash(r))
...
class CBlock(CBlockHeader):
    ...
    def solve(self, skip = True):
        # solve check is switched off in regtest network;
        # to minimize code modifications, let it be parameter (skip = True)
        self.rehash()
        if skip:
            return
        target = uint256_from_compact(self.nBits)
...
class NodeConn(asyncore.dispatcher):
    ...
    MAGIC_BYTES = {
        "mainnet": b"\xfb\xe0\xb6\xd9",   # mainnet
        "testnet3": b"\xfd\xd0\xc8\xef",  # testnet3
        "regtest": b"\xf0\xbd\xb5\xd8",   # regtest
    }
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to PLC Ultima Node: %s:%d' % (self.dstaddr, self.dstport))
...
def extract_total_amount_from_input(input_hex):
    input = FromHex(CTxIn(), input_hex)
    assert_equal(input.prevout.n, TXIN_MARKER_TOTAL_AMOUNT)
    data_len = int(input.scriptSig[0])
    data = input.scriptSig[1:]
