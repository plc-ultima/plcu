...
def sign_compact(digest, priv_key, compressed = True):
    ...
    if len(priv_key) != 32 and len(priv_key) != 33:
        raise AssertionError('priv_key must be 32 or 33 bytes long ({} provided)'.format(len(priv_key)))
    # compressed = (len(priv_key) > 32 and priv_key[32] == 1) # let it be as input parameter
    priv_key = priv_key[0:32]
...
def recover_public_key(digest, signature, compressed = True):
    ...
    if not ecdsa.VerifyingKey.from_public_point(Q, curve=ecdsa.SECP256k1).verify_digest(signature, digest, sigdecode=ecdsa.util.sigdecode_string):
        return None
    key_uncompressed = b'\x04' + ecdsa.VerifyingKey.from_public_point(Q, curve=ecdsa.SECP256k1).to_string()
    if not compressed:
        return key_uncompressed
    pubkey = CECKey()
