...
def GetP2PKHScriptWithTimeLock(pubkeyhash, time):
    return CScript([time, OP_CHECKLOCKTIMEVERIFY, OP_DROP, OP_DUP, OP_HASH160, pubkeyhash, OP_EQUALVERIFY, OP_CHECKSIG])

def GetP2PKHScriptWithTimeLockEx(pubkeyhash, lock_time, active_time):
    ss = bytes()
    ss += struct.pack("<i", lock_time)
    ss += struct.pack("<i", active_time)
    full_time = struct.unpack("<Q", ss)[0]
    return CScript([full_time, OP_CHECKLOCKTIMEVERIFY, OP_DROP, OP_DUP, OP_HASH160, pubkeyhash, OP_EQUALVERIFY, OP_CHECKSIG])
...
def GetAbMintingMultisigScript(key1_police, key2A_user, key2B_project):
    return CScript([OP_DUP, OP_HASH160, hash160(key1_police.get_pubkey()), OP_EQUAL, OP_IF, OP_CHECKSIG, OP_ELSE,
                    OP_OVER, OP_HASH160, hash160(key2A_user.get_pubkey()), OP_EQUALVERIFY, OP_DUP, OP_HASH160,
                    hash160(key2B_project.get_pubkey()), OP_EQUALVERIFY, OP_2, OP_CHECKMULTISIG, OP_ENDIF])

def GetAbMintingLockedMultisigScript(key_user, project_key, timepoint):
    return CScript([OP_DUP, OP_HASH160, hash160(key_user.get_pubkey()), OP_EQUAL, OP_IF, struct.pack("<I", timepoint),
                    OP_CHECKLOCKTIMEVERIFY, OP_DROP, OP_CHECKSIG, OP_ELSE,
                    OP_OVER, OP_HASH160, hash160(key_user.get_pubkey()), OP_EQUALVERIFY, OP_DUP, OP_HASH160,
...
def ScriptAddress(script, testnet = True):
    return AddressFromScriptHash(hash160(script), testnet)
