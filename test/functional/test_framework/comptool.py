...
class TestManager(object):
    ...
    def sync_blocks(self, blockhash, num_blocks):
        ...
        def tell_details():
            return 'blockhash: {}, nodes: {}'.format(blockhash, [blockhash in node.block_request_map and node.block_request_map[blockhash] for node in self.test_nodes])

        # --> error if not requested
        wait_until(blocks_requested, attempts=20*num_blocks, lock=mininode_lock, print_func=tell_details)
            ...
            for c in self.connections:
                if outcome is None:
                    if c.cb.bestblockhash != self.connections[0].cb.bestblockhash:
                        logger.error('Blocks: %064x vs %064x' % (c.cb.bestblockhash, self.connections[0].cb.bestblockhash))
                        return False
                elif isinstance(outcome, RejectResult): # Check that block was rejected w/ code
                    if c.cb.bestblockhash == blockhash:
                        logger.error('Block %064x was not rejected, expected outcome: %s' % (blockhash, outcome))
                        return False
                    ...
                    logger.error('Blocks: %064x and %064x, outcome: %s' % (c.cb.bestblockhash, blockhash, str(outcome)))
                    return False
            return True
