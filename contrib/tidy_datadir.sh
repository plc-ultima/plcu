...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete PLC Ultima database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no PLC Ultima datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old PLC Ultima datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected PLC Ultima 0.7 datadir."
    ;;
  3)
    echo "Detected PLC Ultima pre-0.8 datadir."
    ;;
  4)
    echo "Detected PLC Ultima 0.8 datadir."
    ;;
esac
